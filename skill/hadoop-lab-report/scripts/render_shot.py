...
# 终端字体:英文(ASCII)用 **DejaVu Sans Mono**、中文用 **Microsoft YaHei UI**。
# DejaVu Sans Mono 本机大概率没装,故 skill 自带 ttf,用 @font-face 让无头浏览器真用上它
# (否则会静默回退到别的等宽字);DejaVu 无 CJK 字形,中文按 font-family 顺序回退到雅黑。
ASSET_FONT = os.path.join(os.path.dirname(HERE), "assets", "fonts", "DejaVuSansMono.ttf")
_FONT_URI = "file:///" + os.path.abspath(ASSET_FONT).replace("\\", "/")
FONT_FACE = (
    "@font-face { font-family:'DejaVu Sans Mono'; font-weight:normal; font-style:normal;\n"
    f"  src:url('{_FONT_URI}') format('truetype'); }}\n"
) if os.path.exists(ASSET_FONT) else ""
# 英文走 DejaVu Sans Mono,中文回退 Microsoft YaHei UI(再退 Microsoft YaHei / 通用等宽)。
FONT_STACK = "'DejaVu Sans Mono','Microsoft YaHei UI','Microsoft YaHei',monospace"
...
# 字体:英文 DejaVu Sans Mono(skill 自带 ttf,@font-face 注入)/ 中文 Microsoft YaHei UI。
CSS = (
    FONT_FACE +
