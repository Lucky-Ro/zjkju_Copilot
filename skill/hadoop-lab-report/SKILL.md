...
### 阶段 -1 — 运行前先自更新 skill(git 拉最新版,免得用了过期版本)
**接任务、动手干之前的第一件事**:跑一次自更新脚本,把本 skill 拉到最新版。
```
python <skill>/scripts/update_skill.py
```
- 用 Claude Code 自带的 git;**默认拉 GitHub,网络超时/失败自动退让 Gitee**(两镜像同源):
  GitHub `https://github.com/Lucky-Ro/zjkju_Copilot` / Gitee `https://gitee.com/lucky_ro/zjkju_Copilot`。
- **安全**:`git pull --ff-only`,绝不覆盖本地未提交改动;非 git 安装则用「缓存 clone + 同步子树」更新,
  只覆盖确有变化的文件、从不删除多余文件。脚本自动判断部署形态,你无需关心。
- **拉失败也别卡住**:退出码 0=已更新/最新;3=两镜像都没拉成(多半网络问题)——这时**直接用当前版本继续**,
...
**FinalShell 默认风格 PNG**:白等宽字、右侧淡滚动条;**不要**画 macOS 窗框/红绿灯/语法高亮(那样很「疏离」)。
...
- `update_skill.py` —— **阶段 -1 自更新**:git 拉最新版,默认 GitHub、超时退让 Gitee;`--ff-only` 不覆盖本地改动;
  非 git 安装走「缓存 clone + 同步子树」。退出码 3=两镜像都没拉成(用当前版本继续即可)。
...
- `render_shot.py` —— 真实终端流 → **FinalShell 风**终端 PNG(**默认叠 FinalShell 壁纸**,`--black-bg` 退回纯黑底白字、
  `--bg-image` 换壁纸;淡滚动条;提示符来自真实流、逐行重放不伪造;**滤掉脚本日志行/`(应答)`/注入横线/zk log4j 噪声**;
  **Edge 失效自动回退 Chrome**;`--prompt` 仅兜底)。
...
1. 可复用、可开源 skill 本体(本目录):`SKILL.md`、`README.md`、参考资料、知识库**生成器**(`build_series_kb.py`,
   ...
   schema 与纯占位示例、学校空白模板 `assets/template.docx`、FinalShell 终端壁纸 `assets/FinalShellBackGround.png`。
2. 验证:完整跑一遍 **P4**(`hadoop-e04`),产出符合模板的 `report.docx` 与 `run.log`;
   `hadoop-training03/04` 可正确解析(子任务/提示/样例代码)并出报告。
