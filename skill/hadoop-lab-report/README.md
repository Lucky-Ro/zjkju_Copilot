...
> 一个 **Claude Code / Claude Agent Skill**:把一篇 [heisun.xyz](https://heisun.xyz/docs/hadoop-e/) 的 Hadoop 实验教程网址丢进来,自动 **读教程 → 在你的虚拟机上 SSH 真跑 → 终端截图 → 按湛江科技学院模板排版成 Word 实验报告**,直到交付。
>
> 覆盖整个 **e0\* 系列(P1–P7)**:部署 Hadoop、HDFS、MapReduce、Hive、HBase、Zookeeper、Flume/Sqoop。换教程、换人都不抓瞎。
...
给定一个教程网址(如 `https://heisun.xyz/docs/hadoop-e/hadoop-e04/`),它会:

1. **解析教程** → 子任务、有序步骤、命令、HiveQL/SQL/XML、交互应答、常见问题(`parse_tutorial.py`)。
...
4. **终端截图**(`render_shot.py`):把命令+输出渲染成深色终端风格 PNG(Windows 自带 Edge 无头,零额外安装)。
...
- 配置 `lab_config.json` 分两步,Claude 会自动驱动,你基本只需在弹窗里填身份:
  1. **教程缺省自动落盘**:`python <skill>/scripts/build_series_kb.py`(通读全系列产出 `./series_defaults.json`)+
     `python <skill>/scripts/collect_config.py --autofill --tutorial <教程URL>`——把教程已给的节点 IP/端口/缺省密码
     **直接写好、不问你**(做实验基本用默认)。
...
└── scripts/
    ├── _common.py                # 共享:配置/打码/路径边界(ensure_outside_skill)/学号/UTF-8
    ...
    ├── render_shot.py            # run.log 终端流 → 终端风格 PNG(忠实重放)
    ├── convert_template.py       # .doc → .docx(Word COM / soffice)
    ├── fill_report.py            # 填进学校模板(从零 --template 或续写 --into)
    ├── build_series_kb.py        # 通读全系列 → 缺省值知识库(产出到项目目录,不进 skill)
